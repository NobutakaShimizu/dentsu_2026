...
- 主テーマ：誤り訂正符号、エクスパンダーグラフをどのように利用するのか？
  - 細かい証明は省略
- 研究集会の主旨「他分野の専門家向けの高度な入門講義」
  <v-click> -> 符号（code）に入門します。</v-click>
...
- 入力がランダム (**平均時**計算量) <-> 任意の入力で計算せよ (**最悪時**計算量)
- 全成分の計算でなくてよい（**近似**アルゴリズム）<-> 全成分を計算せよ (**厳密**アルゴリズム)
...
- **最悪時厳密から平均時近似への帰着**: ランダムな入力で近似的に解ける $\Rightarrow$ 任意の入力で厳密に解ける
